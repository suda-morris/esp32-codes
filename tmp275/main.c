...
#define TMP275_SENSOR_ADDR  			0x48	/*!< slave address for TMP275 sensor */
...
xSemaphoreHandle print_mux; /*!< semaphore to protect printing content */
...
static esp_err_t i2c_master_write_slave(i2c_port_t i2c_num, uint8_t* data_wr,
	...
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, ( ESP_SLAVE_ADDR << 1) | WRITE_BIT,
	ACK_CHECK_EN);
 ...
 * | start | slave_addr + wr_bit + ack | write 1 byte + ack | start | slave_addr + rd_bit + ack | read 1 byte + ack  | read 1 byte + nack | stop |
 * --------|---------------------------|--------------------|-------|---------------------------|--------------------|--------------------|------|
 */
static esp_err_t i2c_master_sensor_test(i2c_port_t i2c_num, uint8_t* data_h,
	...
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, TMP275_SENSOR_ADDR << 1 | WRITE_BIT,
	ACK_CHECK_EN);
	i2c_master_write_byte(cmd, TMP275_CONFIG_REG, ACK_CHECK_EN);
	...
	i2c_master_write_byte(cmd, TMP275_SENSOR_ADDR << 1 | WRITE_BIT,
	ACK_CHECK_EN);
	i2c_master_write_byte(cmd, TMP275_TEMP_REG, ACK_CHECK_EN);
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, TMP275_SENSOR_ADDR << 1 | READ_BIT,
	ACK_CHECK_EN);
...
static void i2c_test_task(void* arg) {
	...
	while (1) {
		...
		printf("*******************\n");
		printf("TASK[%d]  MASTER READ SENSOR( TMP275 )\n", task_idx);
		printf("*******************\n");
		if (ret == ESP_OK) {
			printf("data_h: %02x\n", sensor_data_h);
			printf("data_l: %02x\n", sensor_data_l);
			printf("sensor val: %.2f°C\n",
					((sensor_data_h << 8 | sensor_data_l) >> 4) * 0.0625);
		} else {
		...
		size_t d_size = i2c_slave_write_buffer(I2C_SLAVE_NUM, data,
		RW_TEST_LENGTH, 1000 / portTICK_RATE_MS);
		if (d_size == 0) {
			printf("i2c slave tx buffer full\n");
			ret = i2c_master_read_slave(I2C_MASTER_NUM, data_rd,
			DATA_LENGTH);
		} else {
			ret = i2c_master_read_slave(I2C_MASTER_NUM, data_rd,
			RW_TEST_LENGTH);
		...
		ret = i2c_master_write_slave( I2C_MASTER_NUM, data_wr,
		RW_TEST_LENGTH);
		if (ret == ESP_OK) {
...
void app_main() {
	...
	xTaskCreate(i2c_test_task, "i2c_test_task_0", 1024 * 2, (void* ) 0, 10,
			NULL);
	xTaskCreate(i2c_test_task, "i2c_test_task_1", 1024 * 2, (void* ) 1, 10,
			NULL);
}
